...
# emits a compact JSON summary on stdout. NEVER reads message text content — counts and
...
# fields default to 0/empty, an unreadable projects dir yields an empty-but-valid summary.
...
printf '%s\n' "$records" | jq -s \
  ...
  | .cache = { cache_read_ratio: ( if (.totals.input + .totals.cache_read) > 0
        then ((.totals.cache_read) / (.totals.input + .totals.cache_read) * 100 | floor) / 100 else 0 end ) }
