...
- `docs/ROADMAP.md` — new `Backlog` item "State-file permission guidance for `state_io.md`" proposing 0600 mode on state files (`profile.json`, `recommendations.json`, etc.) on POSIX systems via `os.chmod` after `os.replace`. Inspiration: claude-remember `scripts/resolve-paths.sh` `umask 077` pattern. Sub-decisions (where to set mode, Windows no-op, backward compatibility, tempfile default behavior) deferred to scheduling cycle.
