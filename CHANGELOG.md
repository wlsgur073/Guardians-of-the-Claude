...
- `docs/ROADMAP.md` — added new `Backlog` item "Hook lifecycle dispatch system" proposing the `hooks.d/<event>/<priority>-<name>.sh` layered pattern as a replacement for the current flat `plugin/hooks/*.sh` layout once hook surface grows. Today the plugin registers two single-purpose hooks (`SessionStart`, `PreToolUse:Bash`), so flat layout is adequate. Entry records explicit schedule triggers (third hook event OR multi-concern single event) and deferred sub-decisions (dispatcher location, priority numbering, no-op semantics, discoverability).
