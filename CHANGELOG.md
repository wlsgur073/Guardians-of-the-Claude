...
- `plugin/references/model-drift-rules.md` (1.4.0 → 1.5.0) — **Claude Fable 5.1 registered** (released 2026-09-01; verified 2026-09-08 against Anthropic's model catalog, the Bedrock and Google Cloud context-window statements, and the Claude Code model-config docs): new `claude-fable-5-1*` / `anthropic.claude-fable-5-1*` rows normalize to `fable` / `1M` / `extended_any` / `compaction_capable` (`current`, `observed`). That is the same fingerprint as Fable 5, so a Fable 5 → 5.1 upgrade is not reported as drift; longest-match routes the 5.1 ID to its own row. `fingerprint_space_version` stays 1.1.0. The Fable 5 rows are relabeled `legacy` (the catalog moved Fable 5 to Legacy models — advisory wording only, recognition unchanged), and the Provider Coverage examples now show current-generation IDs. `ci/fixtures/t3-model-drift/test-cases.json` gains Fable 5.1 cases for both providers (25 → 27; all pass).
- `plugin/references/lib/model-prices.json` — a `claude-fable-5-1` row ($10/$50; cache read $0.25 because Fable 5.1 bills cache hits at 0.025x input instead of the usual 0.1x; cache write $12.50) and a `claude-haiku-4-5-20251001` row (the pinned snapshot ID the API reports for the `claude-haiku-4-5` alias, which previously fell through to the $0 default).
- `docs/guides/advanced-features-guide.md` (1.4.1 → 1.4.2) — the agent Model Selection table gains the `fable` tier (Claude Code accepts `fable` as a subagent `model:` value and as a `/model` alias), and the `/create` advanced template's model menu and the `/optimize` Agent Model Diversity step now offer `fable` alongside haiku / sonnet / opus — framed as an escalation for work where `opus` at high effort falls short, not a default.
...
- `plugin/references/lib/model-prices.json` — added current-generation price rows so local estimates stop falling through to the $0 default: `claude-fable-5` ($10/$50), `claude-opus-5` ($5/$25), `claude-sonnet-5` ($3/$15 standard, effective 2026-09-01; the intro price $2/$10 through 2026-08-31 is recorded in the file note — superseded by the Fixed entry below: the increase never took effect). Sonnet 4.6 and Haiku 4.5 rows verified correct and unchanged.
- `docs/guides/settings-guide.md` (1.3.1 → 1.3.2) — auto mode's supported models now read "any Fable model" (Fable 5.1 / Fable 5) instead of "Fable 5", with the unsupported set (Sonnet 4.5, Opus 4.5, Haiku, Claude 3) spelled out; the fast-mode paragraph is re-dated to September 2026 and states that fast mode covers Opus 5 / Opus 4.8 only (no Fable, Sonnet, or Haiku) and is not offered on Bedrock, Google Cloud, Foundry, or Claude Platform on AWS; the model-governance line lists the current alias targets (`fable` → Fable 5.1, `opus` → Opus 5 (Opus 4.6 on Foundry), `sonnet` → Sonnet 5, `haiku`, `best`) and the `ANTHROPIC_DEFAULT_*_MODEL` pins. `plugin/references/security-patterns.md` carries the same auto-mode wording.
- `plugin/references/model-drift-rules.md` — the `claude-opus-4-7@*`, `claude-opus-4-6@*`, and `claude-sonnet-4-6@*` rows are removed as ID shapes that never shipped: from the 4.6 generation on every model ID is dateless and Google Cloud exposes the bare first-party ID (the same reasoning that removed the Opus 4.8 parity row in 1.4.0; a dated input still normalizes through the wildcard row to the same fingerprint, and the three t3 cases now document that). Active rows 23 → 22 net of the two Fable 5.1 additions. The Provider Coverage → Vertex section states which models actually ship dated IDs (the 4.5 generation and earlier; rows exist for Sonnet 4.5 and Haiku 4.5), the Claude 5 family note's Sonnet 5 release date is corrected to 2026-06-30, and the Microsoft Foundry note is revisited: Foundry deployments now default to the first-party model ID, which the Anthropic-direct rows already match — only a renamed deployment falls through to `null`. The Google Cloud Sonnet 4.5 row stays `1M`: Google's model card still lists 1M (Preview) beside 200k (GA) while Anthropic's Google Cloud page lists Sonnet 4.5 as 200k — the note now records both sources and the rule the table applies (a provider preview counts until the provider retires it).
...
- `plugin/references/lib/model-prices.json` — the Claude Sonnet 5 row is corrected to `$2/$10` (cache read `$0.20`, cache write `$2.50`): Anthropic made the launch price permanent, and the increase to `$3/$15` scheduled for 2026-09-01 — which the row had pre-applied — did not occur (verified 2026-09-08). The usage-parser goldens are unaffected (the fixture sessions contain no Sonnet 5 usage).
- `docs/guides/advanced-features-guide.md` — the "`haiku` is ~60x cheaper than `opus`" cost claim dated from the Claude 3 generation; at current list prices Haiku 4.5 is ~5x cheaper than Opus 5 and ~10x cheaper than Fable 5.1, and the guide now says so.
- `plugin/skills/audit/references/output-format.md` — the drift-advisory axis-rendering rule listed `opus` / `sonnet` / `haiku` as the user-facing `family_tier` values; `fable` (fingerprint space 1.1.0) is added so an `opus → fable` transition has a documented rendering.
...
- `ci/fixtures/t3-model-drift/test-cases.json` — the Bedrock Sonnet 4.6 expectation tracks the table's `200k` → `1M` update, and five new cases cover the Claude 5 family rows (Opus 5 / Sonnet 5 on Anthropic-direct and Bedrock) plus the deliberate `claude-fable-5` → `null` fail-safe.
...
- Optional `guardians/config.json` customization, deep-merged across plugin defaults, `~/.claude/guardians/config.json`, and per-project `<project>/.claude/guardians/config.json`. `/secure` honors `secure.additional_deny_patterns` (added to existing deny rules, never removed); `/optimize` honors `optimize.skip` (categories to omit from proposals). With no config file present, behavior is unchanged. New `plugin/references/lib/config-resolve.sh` helper and `guardians-config` JSON schema.
...
- `plugin/references/tool-description-quality.md` — new Principle 5 (Tool hierarchy positioning). Surgical tools described as primary defaults; fallback tools (Bash) should carry explicit "when to use this instead" guidance, since agents otherwise default to the most-general tool and bypass tool-level permission scopes.
...
- `templates/advanced/CLAUDE.md` — new `## Verification` subsection between Workflow and Project Structure with read-back-after-edit, tool-success ≠ task-correct, and scope-checked reporting rules; cross-link to `verification-discipline.md` DoD checklist template. Closes the Quality Contract anchor in the advanced template's edit envelope.
- `plugin/skills/create/SKILL.md` — new `## Contract Framework` section between Route to Path and Phase 3.5 enumerating the four agent-contract patterns (Identity-DNA, memory patterns, three permission tiers, verification discipline) that path files (`templates/starter.md`, `templates/advanced.md`) embed when generating CLAUDE.md and settings.json content. Acts as a discoverability anchor for future skill maintenance.
- `plugin/references/model-drift-rules.md`, `ci/fixtures/t3-model-drift/test-cases.json`, `ci/README.md` — registered Claude Opus 4.8 (released 2026-05-28) in the model-drift normalization table: three provider rows (`claude-opus-4-8@*` Vertex, `claude-opus-4-8*` Anthropic-direct, `anthropic.claude-opus-4-8*` Bedrock), all normalizing to the `opus` / `1M` / `extended_any` / `compaction_capable` fingerprint, matching the existing Opus 4.6/4.7 multi-provider pattern. Anthropic-direct and Bedrock are GA at launch; a table note records that Vertex publisher-model exposure may lag the launch by a short window. Added three matching t3 conformance cases (16 → 19) and bumped `model-drift-rules.md` frontmatter `version` 1.0.0 → 1.1.0 (`fingerprint_space_version` unchanged — the 4-axis fingerprint space is the same).

### Changed

- `docs/guides/claude-md-guide.md` — the emphasis sub-section under Pruning now defines *which* rules earn rigid `IMPORTANT`/`YOU MUST` phrasing: match a rule's form to the cost of an error (rigid when you need guaranteed, predictable compliance; otherwise state the *why* and let judgment adapt). Bumped `version` 1.4.2 → 1.4.3. (Ref: Anthropic, "Claude's constitution".)
- `docs/guides/trustworthy-agents-guide.md` — Value Alignment gains a high-stakes carve-out: rationale-first is the default, but severe or hard-to-reverse rules also warrant rigid phrasing (rationale and rigidity are not opposites). Bumped `version` 1.2.2 → 1.2.3.
- `docs/guides/mcp-guide.md` — Security Considerations now distinguish pinned/reviewed local servers (auditable snapshots) from `npx -y` (remote supply-chain behavior) and from remote/hosted connectors (can change behavior after approval); added "vet new connectors with fake data + minimal scope first" guidance; corrected the intro line that implied all MCP servers run locally. Bumped `version` 1.0.3 → 1.0.4. (Ref: Anthropic, "How we contain Claude".)
- `plugin/references/security-patterns.md` — extended the `data-exfiltration` threat with an approved-domain caveat (a domain allowlist is a capability grant, not a destination filter); qualified the "auto + sandbox = strongest practical profile" line accordingly; added a "signal, not silence" note on approval fatigue under Permission and Safety Decision Principles; extended the Defense Surfaces MCP row with the local-vs-remote posture. (Ref: Anthropic, "How we contain Claude".)
- `docs/guides/trustworthy-agents-guide.md` — Subagent Observability gains a "Trust, not just visibility" note: sub-agent output is evidence, not a higher-trust source (multi-agent trust escalation), cross-linked to the multi-agent peer-message protocol. Bumped `version` 1.2.1 → 1.2.2.
- `docs/guides/effective-usage-guide.md` — Output Discipline now frames terseness as a bias, not a hard cap, citing the April 2026 Claude Code postmortem's measured ~3% coding-intelligence drop from a rigid per-reply word limit. Bumped `version` 1.8.2 → 1.8.3.
- `docs/guides/claude-md-guide.md` — Writing Principles gains a "prefer model-agnostic rules" principle: scope model-dependent rules to a named version and revisit on `/audit` drift. Bumped `version` 1.4.1 → 1.4.2.
- `docs/guides/settings-guide.md` — Permission Modes and Safety gains a note that reasoning effort is a runtime quality↔latency dial (not a `settings.json` key); prefer higher effort for hard/security-sensitive/long-running work. Bumped `version` 1.2.2 → 1.2.3.
...
- `docs/guides/effective-usage-guide.md` — new `### Operational verification` subsection added INSIDE the existing `## The #1 Practice: Give Claude a Way to Verify Its Work` section. Names the three concepts from verification-discipline.md (read-back-after-edit, tool-success ≠ task-correct, scope-checked reporting) and points readers at the full rubric for the runtime gap that automated tests don't cover.
- `plugin/skills/audit/SKILL.md` — Phase 5 Critical Thinking section now also cross-references `verification-discipline.md` for the operational-verification rubric. Audit findings should apply read-back when citing specific lines from audited files; complementary to the Socratic discipline above.
- `templates/advanced/sprint-contract.md` — new "See also" cross-link to `verification-discipline.md` DoD checklist template; sprint-contract remains a scope artifact (not conflated with quality artifact).
- `templates/advanced/CLAUDE.md` — new `## Memory` section between Code Style and Development Approach with TaskFlow-flavored memory examples (user / feedback / project / reference entries), what-NOT-to-save guidance, and cross-link to the new memory-patterns guide. Trust Boundary section also gains a new `**Action-tier reference.**` paragraph linking to the three-tier permission model in `settings-guide.md`.
...
- `templates/advanced/.claude/CLAUDE.md` — alternative-location description extended to enumerate the canonical content blocks (Identity-DNA, Trust Boundary, Memory framework, Development Approach, Verification) that live in the root template. Clarifies which content is duplicate-by-design when picking this placement; reinforces the "pick one, not both" pair pattern documented in the file itself.
- `docs/guides/claude-md-guide.md` — the `Updating Mid-Session` paragraph now cross-links to `memory-patterns-guide.md` for the auto-memory entry schema (four types, frontmatter, `MEMORY.md` index format, verify-before-recommending discipline). Wires `/memory` mid-session guidance into the canonical memory-patterns documentation.
- `docs/guides/getting-started.md` — Step 3 now includes a third cross-link, to `plugin/references/verification-discipline.md`, for projects shipping from a CLI editor (file diff is the deliverable). Pairs with the existing Identity-DNA and memory-patterns cross-links to make all four agent-contract patterns discoverable from the entry-point guide.
- `plugin/skills/secure/SKILL.md` — Phase 1.4 Autonomy Risk Policy preamble now cross-links to `settings-guide.md § The three permission tiers`. Clarifies that the `allow` / `ask` / `deny` entries this skill scans and mutates correspond to the Regular / Explicit-permission / Prohibited tiers, and that Phase 3.4 tightening logic moves entries between those tiers.
...
- `plugin/references/verification-discipline.md` — evidence-label vocabulary normalized for internal consistency: the DoD checklist's `publicly-documented` (line 67) and the self-policing meta-loop's `publicly-documented` / `speculative-needs-justification` (line 81) now use `public-doc-backed` and `speculative`. Both lists now share a single canonical label set, preventing downstream consumers from receiving different vocabularies depending on which checklist they reference.

## [3.0.1] - 2026-05-26

### Added

- `docs/ROADMAP.md` — new `Backlog` item "3-tier config deep merge for skill rule customization", proposing a `jq`-based deep merge of plugin-bundled defaults + `~/.claude/guardians/config.json` user-global + per-project `<project>/.claude/guardians/config.json` to let users customize `/audit` score gates, `/secure` deny patterns, and `/optimize` skip rules without forking. Sub-decisions (schema versioning, migration, conflict resolution, skill-side opt-in) deferred to scheduling cycle.
- `.github/dependabot.yml` — weekly automated PRs for `github-actions` ecosystem updates. Covers `actions/checkout`, `actions/setup-python`, `actions/cache`, `lycheeverse/lychee-action`, and `ludeeus/action-shellcheck` (the SHA-pinned shellcheck action surfaces version bumps separately). `open-pull-requests-limit: 5` caps PR noise; `commit-message.prefix: "ops"` keeps routine maintenance PRs grouped under one prefix.
- `.github/PULL_REQUEST_TEMPLATE.md`, `.github/ISSUE_TEMPLATE/bug_report.md`, `.github/ISSUE_TEMPLATE/feature_request.md`, `.github/ISSUE_TEMPLATE/config.yml` — contribution onboarding templates. PR template surfaces local validator sweep, smoke-fixture, and guide line-count checks plus the per-file YAML frontmatter `version:` bump rule from CLAUDE.md. Bug report template captures install method, project state, OS / `bash` / `jq` versions, and plugin version path. Feature request template enforces the no-per-stack-template / no-architecture-guidance scope boundaries from CLAUDE.md and routes contract-changing proposals through GitHub Discussions first. Issue config disables blank issues and redirects security vulnerabilities to a private GitHub Security Advisory channel.
- `.github/scripts/check-hook-surface-sync.py` — new CI validator that verifies the README's "runs N Claude Code hook(s)" claim matches the event count registered in `plugin/hooks/hooks.json`. The check pattern is narrow (only the contractual "runs N Claude Code hook(s)" phrase is matched, not broader references like "the SessionStart hook") to avoid false positives.
- `.github/workflows/docs-check.yml` — new `hook-surface-sync` job running `check-hook-surface-sync.py`. Brings the docs-check matrix to 21 jobs.
- `docs/ROADMAP.md` — new `Backlog` item "State-file permission guidance for `state_io.md`" proposing 0600 mode on state files (`profile.json`, `recommendations.json`, etc.) on POSIX systems via `os.chmod` after `os.replace`. Sub-decisions (where to set mode, Windows no-op, backward compatibility, tempfile default behavior) deferred to scheduling cycle.
- `.github/workflows/smoke.yml` — new `smoke-windows` job running on `windows-latest` with `shell: bash` (Git Bash from Git for Windows preinstalled on the runner) and `choco install jq` for the `jq` dependency. Catches the platform gotcha class (CRLF normalization, jq version differences, bash-on-Windows path resolution recorded in CLAUDE.md "Cross-platform shell/fixture gotchas") that the Linux-only smoke lane misses. Higher value after v3.0.0 made bash a hard requirement. Brings the smoke workflow to 3 jobs (smoke, smoke-windows, verifier-drift-tripwire).
...
- `docs/ROADMAP.md` — added new `Backlog` item "Hook lifecycle dispatch system" proposing the `hooks.d/<event>/<priority>-<name>.sh` layered pattern as a replacement for the current flat `plugin/hooks/*.sh` layout once hook surface grows. Today the plugin registers one single-purpose hook (`SessionStart`) with a Windows `.cmd` fallback, so flat layout is adequate. Entry records explicit schedule triggers (third hook event OR multi-concern single event) and deferred sub-decisions (dispatcher location, priority numbering, no-op semantics, discoverability).
- `README.md` — added new `Trust Model` section between `Philosophy` and `Day 1 Quickstart`. Describes the plugin's privilege scope (one `SessionStart` hook running with the user's shell privileges, no external LLM calls, no telemetry, no writes outside the project directory or plugin cache). Cross-references `docs/SECURITY.md` for vulnerability reporting and per-skill `SKILL.md` for per-skill privilege scope.
- `plugin/hooks/session-start.sh` — strengthened error handling from `set -e` to `set -euo pipefail` so unset-variable use and silent pipeline failures (e.g., a broken `jq` mid-pipe) become fail-fast instead of producing partial digests. Existing default-expanded references (`${SMOKE_PINNED_UTC:-}`) already conform to `set -u` semantics; no other variable usage needed changes.
- `docs/ROADMAP.md`, `CHANGELOG.md` `[Unreleased]`, `.github/scripts/check-hook-surface-sync.py` — rephrased explanatory framing on recent backlog items, changelog items, and the new validator's docstring so each entry stands on its own terms. Backlog items, changelog items, scheduling triggers, and validator check logic are unchanged; the state-file permission backlog item gains a short rationale paragraph explaining the multi-user host threat directly.

### Fixed

- `README.md` — corrected Trust Model section's hook surface claim from "runs two Claude Code hooks (`SessionStart`, `PreToolUse:Bash`)" to "runs one Claude Code hook (`SessionStart`)". The plugin manifest (`plugin/hooks/hooks.json`) registers only `SessionStart`; the `PreToolUse:Bash` hook referenced was this repo's gitignored user-level dev environment hook (`.claude/hooks/check-commit-msg.py`), not a plugin-registered surface. Caught by the new `check-hook-surface-sync.py` validator before the next release.
- `docs/ROADMAP.md` — corrected the same hook surface drift in the `Hook lifecycle dispatch system` backlog entry: "(two single-purpose scripts)" describing `SessionStart` + `PreToolUse:Bash` rewritten to "(one single-purpose script with a Windows `.cmd` fallback)" describing `SessionStart` alone. Same drift class as the README Trust Model fix above; not caught by `check-hook-surface-sync.py` because the validator scope is README only (extension to ROADMAP deferred until drift recurs).
...
- `plugin/hooks/session-start.ps1`: the unresolved-recommendations digest now selects the oldest `first_seen` with a stable, array-order tie-break (manual min-scan) instead of `Sort-Object first_seen`, which is not a stable sort in Windows PowerShell 5.1. When multiple PENDING recommendations shared a `first_seen` date, the PowerShell hook could name a different `oldest` recommendation than the bash hook (`jq sort_by`, stable), diverging from the shared `sessionstart-orchestrator` goldens. The bash hook was already correct; only the PowerShell side changed. The divergence escaped CI because the PowerShell `sessionstart` smoke lane is skipped on Linux runners (no `pwsh`).
...
- Defense Surfaces Catalog section in `plugin/references/security-patterns.md` — maps input surfaces to existing threats and defensive postures with an "Authorized Security Work" footnote. Addresses ROADMAP backlog item "Defense Surfaces checklist for security reference".
- `templates/starter/CLAUDE.md` and `templates/advanced/CLAUDE.md` (with ko-KR and ja-JP mirrors): new "Untrusted input rule" appended under the existing Trust Boundary section, instructing agents to treat content from 11 input surfaces (repository files, dependency scripts, shell output, browser content, MCP responses, generated artifacts, quoted/pasted external content and attachments, hook code and hook output, persistent local state, CI fixtures, external downloads) as evidence, not instruction. References the Defense Surfaces Catalog in `plugin/references/security-patterns.md` for the canonical surface list and defensive postures. Addresses ROADMAP backlog item "Universal untrusted-content rule for templates".
- `plugin/skills/secure/SKILL.md`: new "Injection Reminder Mapping" sub-section appended under Phase 3 (Fix Selected Items) — maps the 6 injection reminder types observed in Anthropic's community-extracted claude.ai source (image content, cyber-action, system-instruction, ethics, IP, long-conversation) to surfaces from the Defense Surfaces Catalog in `plugin/references/security-patterns.md` and to `/secure` check patterns. 2 of 6 produce mechanical `/secure` checks; 4 are advisory references. Addresses ROADMAP backlog item "Injection-reminder threat-model → `/secure` skill mapping".
...
- `plugin/skills/secure/SKILL.md` Injection Reminder Mapping sub-section: promoted heading from H4 to unnumbered H3 to match Phase 3's existing unnumbered sub-section pattern (Deny Patterns / Security Rule File / File Protection Hooks); tightened the Defense Surfaces Catalog link text by dropping the redundant repo-path prefix; disambiguated cross-references in the System-instruction warning row ("Phase 1.3 + 3 File Protection Hooks" → "Phase 1.3 scan + Phase 3 'File Protection Hooks' sub-section"; "Phase 3.4 suggestion" → "Phase 3.4 4b suggestion"). No mapping or surface-label change.
