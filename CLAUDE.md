...
- 5 additional Python validators in `.github/scripts/` for release-time sweep (not in routine local checks): `check-recommendation-registry.py`, `check-skill-stability.py`, `check-qa-report-shape.py`, `check-hook-script-parity.py`, `check-tag-sha-propagation.py` (post-tag-push only). Routine local checks now also include `check-readme-badge-sync.py` and `check-changelog-anchor-slug.py`. All 11 must pass GREEN before tag/push (tag-sha-propagation runs after the push as the post-tag verifier).
