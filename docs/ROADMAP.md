...
- **3-tier config deep merge for skill rule customization** — adopt the `jq -s 'reduce .[] as $x ({}; . * $x)'` pattern (~5 lines bash) to deep-merge three config sources: plugin-bundled defaults → user-global (`~/.claude/guardians/config.json`) → per-project (`<project>/.claude/guardians/config.json`). Use case: customize `/audit` minimum score gates, `/secure` additional deny patterns, `/optimize` skip rules per-user or per-project without forking the plugin. Sub-decisions when scheduled: config schema versioning (align with `plugin/references/schemas/` policy), migration path (legacy 0-tier → 3-tier), conflict resolution when keys overlap (project wins per merge order), skill-side opt-in (each skill chooses which keys it honors), and `jq` dependency check (already required for SessionStart hook).
- **Hook lifecycle dispatch system** — adopt the `hooks.d/<event>/<priority>-<name>.sh` pattern (priority-ordered shell scripts under one directory per lifecycle event) to replace the current flat `plugin/hooks/*.sh` layout once hook surface grows. Today the plugin registers only `SessionStart` (one single-purpose script with a Windows `.cmd` fallback), so flat layout is adequate. Schedule trigger: adding a third Claude Code hook event (e.g., `SessionEnd`, `UserPromptSubmit`, `PreCompact`), OR a single hook event needing multiple distinct concerns (e.g., session-start auto-brief plus telemetry plus project-detection becoming three separable scripts). Sub-decisions when scheduled: dispatcher script location (top-level `hooks/dispatch.sh` vs per-event), priority numbering convention (`00-99`), no-op semantics for empty event directories, `.gitkeep` vs README-per-directory for discoverability.
...
- **State-file permission guidance for `state_io.md`** — extend `plugin/references/lib/state_io.md`'s atomic write idiom with restrictive 0600 mode for state files (`profile.json`, `recommendations.json`, `config-changelog.md`, `drift-state.json`, `state-summary.md`) on POSIX systems via `os.chmod` after `os.replace`. The rationale matches a shell `umask 077` for write-time access control: prevent state files from being world/group-readable on multi-user hosts (shared dev box, CI runner, jumphost) where the default 0022 umask would leak profile contents to any local user. Sub-decisions when scheduled: where to set the mode (in `atomic_write` itself vs caller), graceful no-op on Windows (POSIX bits not enforced), backward compatibility for existing state files (chmod on first write vs migration script), and which Python `tempfile.NamedTemporaryFile` parameters yield the desired permission (`tempfile` defaults to 0600 on POSIX so the lightest-weight implementation is `os.chmod` after `os.replace`).
