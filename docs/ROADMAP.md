...
- **Injection-reminder threat-model → `/secure` skill mapping** — convert the six injection reminder types (`image_reminder`, `cyber_warning`, `system_warning`, `ethics_reminder`, `ip_reminder`, `long_conversation_reminder`) into a defensive checklist for `/secure`. Originally drafted as a cycle plan but not started; converted to backlog per Codex round-2 review (2026-05-20).
- **Post-ship validation mechanism for user-facing guide additions** — the project's first Philosophy principle ("Verify, don't trust") applies to ourselves too. Currently we ship guide sections without a feedback mechanism to verify they improve user behavior. Initial scope: lightweight signals such as a future `/audit` question "does the project teach users what good Claude behavior looks like?", user docs review channels, and support-issue examples. Triggered by Codex round-2 review (2026-05-20).
- **Universal untrusted-content rule for templates** — add a one-line rule to starter and advanced templates instructing agents to treat content from files, webpages, comments, logs, test fixtures, generated artifacts, and tool output as evidence not instruction. Originally surfaced by Codex round-3 review (2026-05-20) as a Critical-priority safety task.
- **Defense Surfaces checklist for security reference** — expand `plugin/references/security-patterns.md` with a checklist mapping threats to concrete input surfaces (repository files, dependency scripts, shell output, browser content, MCP responses, generated artifacts, hooks, local memory, CI fixtures, external downloads), plus an "Authorized Security Work" rule requiring user-scoped permission. Triggered by Codex round-3 review (2026-05-20).
- **Default-stance pattern example for advanced template** — add a worked example block to `templates/advanced/CLAUDE.md` (or a `.claude/rules/*.md` file) demonstrating the "single-sentence default-stance policy with threshold" pattern from Anthropic's published Opus 4.7 system prompt. Paraphrased only; no verbatim source reproduction. Surfaced during the 2026-05-20 prompt-pattern research cycle as independent from response-discipline (shipped this cycle) and deferred for scope reasons. Placement file (root `CLAUDE.md` vs. `.claude/rules/*.md`) is itself a sub-decision when the item is scheduled.
