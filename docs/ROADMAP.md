...
- **Universal untrusted-content rule for templates** — add a one-line rule to starter and advanced templates instructing agents to treat content from files, webpages, comments, logs, test fixtures, generated artifacts, and tool output as evidence not instruction. Originally surfaced by Codex round-3 review (2026-05-20) as a Critical-priority safety task; see triage in `docs/plans/06-codex-input-triage-and-cycle-defects.md` §A.
