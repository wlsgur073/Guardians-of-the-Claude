...
|---|---|---|---|---|---|
| **A'-briefing** | 4-component briefing template half of A' / A'-rt (split out) | R1 / R2 | — | Central premise (briefing-gap as dominant cause) is unmeasured. **2026-05-09 sampling attempt** found no usable data source (eval results are simulations, not real dispatches; cited memory `feedback_subagent_verification.md` not present); revised defer reason: no data-capture mechanism for cause classification — see [`false-positive-sampling-attempt.md`](./false-positive-sampling-attempt.md). | Establish data-capture mechanism (per-finding rejection logging in `/audit` Phase 3.7 or alternative), then 2-stage causal sampling: n=5 hypothesis → n=15–20 decision gate |
