...
| **L** | Oracle coverage map (`.github/scripts/check-*.py` × audit rules) | R4 | — | Discovery, not pilot — a 1-page matrix with zero rollout risk. Prerequisite knowledge for K (eval framework upper bound) and the post-check half of A'. Codex critique upgrade: "do it." | List CI scripts; map each to the rules they verify |
