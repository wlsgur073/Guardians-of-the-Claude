...
**Symbol density is a failure axis separate from length.** A short file can still mislead if it leans on opaque codes, internal IDs, or terse directives with no rationale: the agent starts reasoning about your symbols instead of the code's actual behavior, producing output that is internally consistent but detached from reality. Length pruning (above) and rationale (the *why* you attach to rules) are independent defenses — a file can be short and still symbol-dense. When project-specific codes or IDs are unavoidable, expand them in plain language on first use, and keep CLAUDE.md a working baseline rather than an index of cross-references.
