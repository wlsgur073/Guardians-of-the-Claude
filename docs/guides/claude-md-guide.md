...
- **`#` prompt shortcut** — When you discover a rule mid-session (e.g., "always run `npm run typecheck` after edits"), prefix the prompt with `#` and Claude proposes adding the learning to CLAUDE.md (you confirm before it writes). Faster than editing the file manually.
