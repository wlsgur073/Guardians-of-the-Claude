...
- **Hook 입력 (stdin)** -- 훅은 stdin으로 이벤트 JSON을 받습니다 (예: `{"tool_input": {"file_path": "..."}}`); `jq -r '.tool_input.file_path // empty'`로 파싱하세요. 경로 관련 환경변수는 **`$CLAUDE_PROJECT_DIR`** (프로젝트 루트) 하나뿐이며 — 파일 경로는 환경변수로 노출되지 *않습니다*
- **`statusMessage`** -- hook 실행 중 UI에 표시되는 텍스트
- **`PreToolUse` + `exit 2`** 는 도구 동작을 차단하고 Claude에게 이유를 알려줍니다 -- `.env`나 마이그레이션 디렉토리 같은 민감한 파일 보호에 사용
...
- **실용 조합:** `SessionStart`로 세션 시작 시 프로젝트 컨텍스트 주입, `PreCompact`로 자동 압축 전 핵심 메모 보존, `SubagentStop`로 agent 결과물을 부모 세션에 반환하기 전 검증
...
**하나의 agent, 하나의 관점.** 구현, 리뷰, 테스트를 하나의 agent에 결합하지 마세요 -- 집중된 역할로 분리하세요.
...
- **`name`** / **`description`** -- skill의 식별자 및 트리거 문구. Claude가 skill 호출 여부를 결정할 때 두 필드를 모두 읽으므로, description은 활성화 기준을 명시적으로 표현해야 합니다 (예: `"Use when the user asks to ..."`)
...
Skill은 컨텍스트를 세 단계로 로드하여 윈도우를 가볍게 유지합니다 — skill **메타데이터**(먼저 로드, 트리거 결정용), SKILL.md **본문**(skill 활성화 시 로드), **보조 파일**(워크플로우 내에서 필요할 때만 로드). skill-local `references/`를 SKILL.md 옆에 묶는 것이 canonical Agent Skills 구조이며, 공유 `plugin/references/*.md`는 콘텐츠가 진정으로 cross-skill인 경우에만 사용하세요.
...
**평가 기반 반복:** 예시 호출과 기대 출력을 먼저 작성한 다음, 그 평가가 통과할 때까지 SKILL.md를 반복 개선하세요. 각 패스 후에는 성공한 접근 방식과 흔한 실수를 Claude가 다시 skill 텍스트에 반영하도록 하세요.

**Description 품질이 중요합니다:** 트리거 문구, 도메인 전문가 관점의 표현, 이중 형식 응답, 실행 가능한 오류 메시지는 Claude가 적절한 타이밍에 skill을 트리거할지 여부에 큰 영향을 미칩니다. 전체 원칙 집합은 [`plugin/references/tool-description-quality.md`](../../../../plugin/references/tool-description-quality.md)를 참고하세요.

**보안:** Skill은 실행 가능한 지시사항입니다 — 신뢰할 수 있는 출처에서만 설치하고, 익숙하지 않은 SKILL.md는 호출 전에 검토하세요. 스크립트를 실행하기 전에 검토하는 방식과 동일하게 다루세요.

> **참고:** 레거시 `commands/` 디렉토리는 deprecated되었습니다. 모든 skill 유형에 `skills/<name>/SKILL.md`를 사용하세요.

### 플러그인 Skill 실전 예시

`guardians-of-the-claude` 플러그인은 역할별 Skill 워크플로우를 보여주는 네 가지 Skill로 구성됩니다:

| Skill | 용도 |
| ----- | ------- |
| `/guardians-of-the-claude:create` | 가이드 기반 설정 마법사 -- CLAUDE.md, settings, rules, 선택적 기능 생성 |
| `/guardians-of-the-claude:audit` | 종합적인 설정 평가 및 점수 산출 |
| `/guardians-of-the-claude:secure` | 보안 개선 -- deny 패턴, 보안 규칙, 파일 보호 hooks |
| `/guardians-of-the-claude:optimize` | 설정 품질 개선 -- rules 분리, agent 다양화, MCP, hook 품질 |

**권장 워크플로우:** `/create` → `/audit` → `/secure` 또는 `/optimize` → `/audit` (재검증). 각 Skill은 다음 Skill로 연결되며, `.claude/.plugin-cache/`의 타임스탬프 파일로 상태를 공유합니다.
