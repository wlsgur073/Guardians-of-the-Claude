...
복잡한 로직에는 인라인 명령 대신 외부 스크립트를 사용하세요. 스크립트는 테스트 가능하고, 버전 관리가 되며, 유지보수가 더 쉽습니다:
...
**타임아웃 가이드라인:** 입력 검증 3-5초, 린트/포맷 10-15초, 빌드/테스트 30초 이상. 스크립트 기반 hook에는 항상 `timeout`을 명시적으로 설정하세요.
