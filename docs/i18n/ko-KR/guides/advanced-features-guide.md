...
Skill은 컨텍스트를 세 단계로 로드하여 윈도우를 가볍게 유지합니다 — skill **메타데이터**(먼저 로드, 트리거 결정용), SKILL.md **본문**(skill 활성화 시 로드), **보조 파일**(`references/`, `examples/`, `scripts/` — 워크플로우 내에서 필요할 때만 로드). skill-local `references/`를 SKILL.md 옆에 묶는 것이 canonical Agent Skills 구조이며, 공유 `plugin/references/*.md`는 비표준입니다 — 콘텐츠가 진정으로 cross-skill인 경우에만 사용하세요.
...
`guardians-of-the-claude` 플러그인은 역할별 스킬 워크플로우를 보여주는 네 가지 스킬로 구성됩니다:
...
| `/guardians-of-the-claude:optimize` | 설정 품질 개선 -- rules 분리, 에이전트 다양화, MCP, hook 품질 |
