...
4. **점진적 강화** — Day 1은 2분 설정. Day 7은 감사와 보안 강화 추가. Day 14는 스킬 간 메모리와 자동 드리프트 탐지가 본격적으로 진가를 발휘합니다. 도구가 사용자와 함께 성장하며, 필요하지 않은 복잡성에 대한 비용은 내지 않습니다.
...
> 자동 생성합니다. 이후 `/guardians-of-the-claude:create`에서 "Existing project"를
> 선택하면 `/init`이 놓친 부분을 채울 수 있습니다.

**여기서 멈춰도 됩니다.** 설정은 자체적으로 동작합니다. 아래 Day 7과 Day 14+ 섹션은 더 원할 때 다음에 일어나는 일을 설명합니다.
...
스킬을 거듭 실행하면서, 플러그인의 **메타 시스템 레이어**가 충실해집니다 — 시간이 지남에 따라 프로젝트에 적응하는 지속적 학습:
...
## v2.11+ 상태 형식 및 stateless mode

**v2.11 마이그레이션** (v2.10.x에서 업그레이드하는 사용자용): 상태(state) 형식이 Markdown 파일에서 JSON으로 변경되었습니다. 업그레이드 후 처음 스킬 실행 시 `local/project-profile.md` + `local/latest-*.md`가 `local/profile.json` + `local/recommendations.json`으로 자동 변환되며, 원본은 `local/legacy-backup/<ISO-8601-UTC>/` 아래 보존됩니다. 단방향 마이그레이션 — 롤백은 수동 복원 + v2.10.x 고정이 필요합니다. 파싱 실패 복구 및 전체 마이그레이션 상세는 [CHANGELOG.md](../../../CHANGELOG.md)의 v2.11.0 엔트리를 참고하세요.

**Stateless mode** (v2.12.0부터): `local/`에 쓸 수 없을 때(읽기 전용 마운트, 개인정보 민감 프로젝트, 사용자 비활성화) 스킬은 일회성 경고를 출력하고 모든 state 파일 쓰기를 건너뜁니다 — 학습 상태가 세션 간에 유지되지 않습니다. 개인정보 민감 프로젝트는 구버전을 고정하는 대신 stateless mode에 의존할 수 있습니다.

**마이그레이션 실패 보고**: [GitHub Issues](https://github.com/wlsgur073/Guardians-of-the-Claude/issues)에 경고 출력과 (가능하면) 파싱 실패 파일의 redacted 스니펫을 첨부해 주세요. 자동 수집되는 텔레메트리는 없습니다.
...
| 4 | [규칙 가이드](guides/rules-guide.md) | CLAUDE.md가 ~200줄을 초과할 때 |
...
Claude Code는 공식(Anthropic 관리) 플러그인과 커뮤니티 플러그인 모두로 기능을 확장할 수 있습니다 — 전체 개발 워크플로우부터 코드 인텔리전스까지. 카테고리별 전체 목록은 **[추천 플러그인 가이드](guides/recommended-plugins-guide.md)**를 참고하세요.
