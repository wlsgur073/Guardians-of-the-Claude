...
**書き込み不可 `local/` の扱い（stateless mode）**: `local/` に書き込めない場合（読み取り専用マウント、プライバシー重視のプロジェクト、ユーザー無効化）、スキルは **stateless mode** に入ります — Step 0.5 が一度だけ警告（`local/ not writable; stateless run — learning disabled`）を出力し、すべての state ファイル書き込み（`profile.json`、`recommendations.json`、`config-changelog.md`、`state-summary.md`、`qa-report.md`）をスキップします。学習状態はセッション間で永続化されません。v2.12.0 で実装され、それ以降のすべてのバージョンで有効です。プライバシー重視のプロジェクトは旧バージョンを固定する代わりに stateless mode に依存できます。
