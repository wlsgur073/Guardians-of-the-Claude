...
**ステートレスモード**（プライバシー重視のプロジェクト）: `local/` が書き込み不可、読み取り専用、または意図的に存在しない場合、スキルはこれを検知してステートレスモードで実行されます（JSON 永続化なし、推奨事項保存なし、`state-summary.md` 生成なし）。一度だけ警告が出力され、スキルの主要出力は正常に完了します。ステートレスモードでは Learning Rules は無効化されます。
