...
例: `guardians-of-the-claude` プラグインは、プロジェクトプロファイル、決定 changelog、蓄積された推奨事項を `local/` サブディレクトリに保存します（`profile.json`、`recommendations.json`、`config-changelog.md`、これらから派生する人間が読める `state-summary.md`）。これらのファイルにより、スキルはセッションをまたいでプロジェクトのコンテキスト・ユーザーの好み・保留中の推奨事項を覚えていられます。
