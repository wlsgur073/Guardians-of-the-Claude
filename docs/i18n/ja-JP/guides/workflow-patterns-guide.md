...
大規模なマイグレーションや分析では、作業を複数の `claude -p` 呼び出しに分散させます。以下の bash ループは呼び出しを*順次*ディスパッチします; 同時実行数を制限して並列化したい場合は `xargs -P` または `ForEach-Object -Parallel` を使ってください。

> **⚠️ コストと安全に関する警告**
>
> - ループ内の `claude -p` は呼び出しごとにトークンコストが発生します。数千ファイル規模のマイグレーションは数時間かかり、相当のコストが累積する可能性があります — スケール前に必ずモデルのトークン単価で見積もってください。
...
# 制限付き並列（ワーカー 4 つ; PowerShell 7+ が必要）
