...
**プロジェクト** (`.claude/settings.json`) -- プロジェクトに関わる全員が使うチーム共有の構成です。よく使うコマンドの権限や、共有する deny ルールなど。このファイルはコミットしてください。（プラグインの**ソース**リポジトリは自身の `.claude/*` を開発専用として gitignore することがあります — このコミット指針は本ガイドに従う**ユーザー**プロジェクトに適用されます。）
...
Linux/WSL2 は `bubblewrap` と `socat` パッケージが必要です。サンドボックスは安全なコマンドを定義された境界内でコマンドごとの承認なしに実行できるようにし、権限プロンプトを削減します — Anthropic 内部の使用上の利点であり、任意環境での保証ではありません。効果的なサンドボックスはファイルシステムとネットワークの両方の隔離が必要です。`denyWrite`/`denyRead`、カスタムプロキシ、セキュリティ上の制限は [sandboxing ドキュメント](https://code.claude.com/docs/en/sandboxing)を参照。
