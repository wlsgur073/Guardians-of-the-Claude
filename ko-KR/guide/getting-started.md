...
**한 번에 설정하고 싶으신가요?** 자동 설정 프롬프트를 사용하세요 -- `/init` 스타일 분석에 더해 rules, 권한 설정, 고급 기능까지 한 번에 생성합니다. 자세한 내용은 [빠른 시작](../README.md#빠른-시작)을 참고하세요.

**새 프로젝트를 시작하나요?** 설정 프롬프트가 빈 프로젝트도 지원합니다. `@setup-prompt.md`를 실행하고 "새로운/빈 프로젝트"를 선택하면 기술 스택 선택부터 안내합니다. 기존 코드가 없어도 됩니다.
