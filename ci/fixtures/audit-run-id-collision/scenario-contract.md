...
description: "Two same-microsecond /audit emissions; the second must bump to max(existing)+1µs (spec §10)"
version: "1.0.0"
---

# Scenario Contract — audit_run_id Canonical Microsecond + Monotonic Bump (spec §10)

This fixture proves the spec §10 `audit_run_id` sub-second fix: when two
...
shell A's retried commit) — the natural same-microsecond collision §10
addresses. The ONLY pre-state difference vs. `state-lock-occ-conflict` is
...
already in the §10 canonical microsecond form (the input itself models the
...
4. **Shell A — Step C (short lock):** re-read `commit_now = commit-0002 !=
   ...
   - A re-runs ONLY the drift-state Update step on that snapshot:
     ...
     2026-04-14T00:00:00.000001+00:00`. This is the §10 collision fix:
     two same-microsecond emissions ⇒ the second becomes max+1µs.
...
- `baseline.audit_run_ids = ["2026-04-13T00:00:00.000000+00:00",
  ...
  forms are uniform — §10 still mandates parse-to-datetime since a mixed
